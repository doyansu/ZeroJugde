...
void dfs(int m,int n,int size,char *list,int *ans,int level){
    //print(list,size);
    if(!m && !n){
        for(int i=0;i<size;++i){
        ...
        for(int i=0;i<size;++i){
            if(checkStraight(list,size,i)){
                if(n){
                    //printf("level=%d enter %d n\n",level,i);
                    list[level*size+i]='+';
                    dfs(m,n-1,size,list,ans,level+1);
                    list[level*size+i]=' ';
                }
                if(m && checkSlash(list,size,level,i)){
                    //printf("level=%d enter %d m\n",level,i);
                    list[level*size+i]='*';
                    dfs(m-1,n,size,list,ans,level+1);
                    list[level*size+i]=' ';
                }
            }
        }
